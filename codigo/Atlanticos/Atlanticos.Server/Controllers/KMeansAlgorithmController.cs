...
namespace Atlanticos.Server.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class KMeansAlgorithmController : Controller
    {
        [HttpGet("api/kMeans")]
        public IActionResult RunKMeansAlgorithm([FromQuery] int? n_clusters)
        {
            ...
            var pythonFilePath = Path.Combine(baseDirectory, "codigo", "algoritmos", "NearestNeighbors.py");
            ...
            var resultJsonPath = Path.Combine(baseDirectory, "codigo", "algoritmos", "outputs", "JSON", "NN_output.json");
